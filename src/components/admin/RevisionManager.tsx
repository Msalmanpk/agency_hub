...
export function RevisionManager({ clients, sessionId }: RevisionManagerProps) {
  ...
  const handleAction = async (revision: Revision & { clientName: string }, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg(data.error || `Failed to ${action} revision.`);
      }
    } catch (e) {
      console.error(`Failed to ${action} revision:`, e);
      setErrorMsg(`Failed to ${action} revision.`);
    } finally {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      {errorMsg && (
        <div className="bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs rounded-lg px-4 py-2.5">
          {errorMsg}
        </div>
      )}
